...
public class AntKillerNeedsManager : MonoBehaviour, INewAntNeeds
{
    // 水相关属性
    ...
    private void Start()
    {
        ...
        if (animator != null)
        {
            Debug.Log($"AntKillerNeedsManager: 成功获取到Animator组件: {animator.name}");
        }
        else
        {
            Debug.LogError("AntKillerNeedsManager: 无法获取Animator组件！");
        }
    ...
    public void FindFoodTarget()
    {
        ...
        {
            GameObject randomPlant = ObjectStatisticsManager.Instance.GetRandomAnimalObject();
            if (randomPlant != null)
            {
    ...
    {
        Debug.Log("AntKillerNeedsManager: PlayEatingAnimation被调用");
        ...
        if (animator != null)
        {
            ...
            Debug.Log($"AntKillerNeedsManager: 开始播放进食动画 - bIsEating设置为true, Animator: {animator.name}");
            ...
            foreach (AnimatorControllerParameter param in animator.parameters)
            {
                ...
                {
                    Debug.Log($"AntKillerNeedsManager: 找到bIsEating参数，类型: {param.type}");
                    break;
                }
            }
        }
        ...
        {
            Debug.LogError("AntKillerNeedsManager: Animator为null，无法播放进食动画！");
        }
    }
    ...
    public void StopEatingAnimation()
    {
        if (animator != null)
        {
            ...
            Debug.Log("AntKillerNeedsManager: 停止进食动画，恢复正常状态");
        }
    }
    ...
    private void CheckEatingAnimationComplete()
    {
        if (isEating && animator != null)
        {
            ...
            {
                Debug.Log("AntKillerNeedsManager: 检测到进食动画完成");
                StopEatingAnimation();
            }
        }
    }
